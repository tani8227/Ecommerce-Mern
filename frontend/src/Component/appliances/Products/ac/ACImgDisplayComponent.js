...
export default function ACImgDisplayComponent(props) {
    ...
    const handleClick = () => {
     if (!isClicked) {
       ...
       handleAddToCart('Appliances',props.ProductCategory, props.id, 'orderpreview');
       setIsClicked(true);
     }
   };
                        ...
                        >
                            ...
                            <Card square elevation={0} sx={{ width:"100%", objectFit:"contain"}}>
                                <CardMedia
                                    ...
                                    image={`${props.img!==undefined&&props.img.length === undefined ? defaultImg : props.img}`}
                                    alt={`iphone-14`}
                                />
                            </Card>
                            <CardActions sx={{ display: "flex", flexDirection: { xs: "row", sm: "row" }, alignItems: { xs: "center" }, justifyContent: { sm: "center" } }}>

                                <Button variant='contained' sx={{ backgroundColor: "#ff9f00",fontSize:"small" }} onClick={()=>{addToCard('Appliances',props.ProductCategory, data._id)}}>
