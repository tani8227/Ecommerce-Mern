...
export default function CoolerImgComponent(props) {
            ...
            <Box padding={0} width={"100%"}>
                ...
                >
                    ...
                    >
                          ...
                          {itemData&& itemData.slice(0, 4).map((ele, index) => (
                            <Item
                                key={ele._id}
                                square
                                elevation={1}
                                sx={{ position: 'relative', justifyContent: { xs: 'center' }, alignItems: { xs: 'center' }, minWidth: "180px" }}
                            >
                                ...
                                <Link to={`${ele._id}`} style={{ textDecoration: 'none' }}>
                                    <Card square elevation={0} sx={{ width: { xs: '180px' } }}>
                                        ...
                                        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '16px', color: 'black' }}>
                                          {`${ele.companyName} ${ele.capacity} ${ele.color} ${ele.coolerType} Air Cooler `}
                                        </Typography>
