                ...
                <Grid item xs={12} sx={{ display: "flex", flexDirection: "column", }}  >
                    <Item square elevation={0}  >
                        <Typography variant='h6' color={'black'} sx={{ padding: 0, margin: 0 }} >
                            ...
                            {`${props.product.product.companyName} ${props.product.product.capacity} ${props.product.product.coolerType} Air Cooler`}
                        </Typography>
                        ...
                        <List  sx={{ padding: 0, margin: 0 }}>
                            <Typography variant='h6' color={'grey'} sx={{ padding: 0, margin: 0, fontWeight: 500 }} >
                                ...
                                <ListItem sx={{ padding: 0, margin: 0, display: 'list-item', listStyleType: 'disc', }}>
                                    <Typography variant='body2' sx={{ color: "black" }}>
                                        {`${props.product.product.capacity} ${props.product.product.capacity && ': Good for couples and small families'}`}
                                    </Typography>
                                ...
                                <ListItem sx={{ padding: 0, margin: 0, display: 'list-item', listStyleType: 'disc' }}>
                                    <Typography variant='body2' sx={{ color: "black" }}>
                                        {`${props.product.product.bodyMaterial} Body`}
                                    </Typography>
                    ...
                    >
                        <Box sx={{ display: "flex", justifyContent: "flex-start", alignItems: "center", flexWrap: 'wrap', gap: 2, }}>
                            ...
                            {allItem && allItem.map((ele, index) => (
                                ...
                                <Link key={ele._id} to={`/Appliances/${props.ProductCategory}/${ele._id}`}>
                                    <Card
