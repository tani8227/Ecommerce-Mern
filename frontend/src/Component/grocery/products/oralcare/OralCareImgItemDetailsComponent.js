...
export default function OralCareImgItemDetailsComponent(props) {
                ...
                <Grid item xs={12} sx={{ display: "flex", flexDirection: "column", }}  >
                    <Item square elevation={0}  >
                        <Typography variant='h6' color={'black'} sx={{ padding: 0, margin: 0 }} >
                            ...
                            {`${props.product.product.companyName} ${props.product.product.modelName} ${props.product.product.type} ${props.product.product.type} `}
                        </Typography>
                            ...
                            <Typography variant='h6' color={'grey'} sx={{ padding: 0, margin: 0, fontWeight: 500 }} >
                                ...
                                <ListItem sx={{ padding: 0, margin: 0, display: 'list-item', listStyleType: 'disc', }}>
                                    <Typography variant='body2' sx={{ color: "black" }}>
                                        {`${props.product.product.modelName}`}
                                    </Typography>
                                ...
                                <ListItem sx={{ padding: 0, margin: 0, display: 'list-item', listStyleType: 'disc', }}>
                                    <Typography variant='body2' sx={{ color: "black" }}>
                                        {`${props.product.product.organicType} ${props.product.product.type}`}
                                    </Typography>
                    ...
                    <Item square elevation={0} sx={{ width: "fit-content" }}>
                        ...
                        {Field && Field.length > 0 &&
                            ...
                            {Field && Field.length > 0 && Field.map((ele, index) => (
                                ...
                                <Link key={ele._id} to={`/Grocery/${props.ProductCategory}/${ele.id}`} style={{ textDecoration: "none", }}>
