            ...
            if(AllItems!==undefined)
                {
                     ...
                     AllItems.map(async (item) => {
                        ...
                        const response = await fetch(`${process.env.REACT_APP_BACKEND_APP_API_URL}/api/v1/user/order/getOrderItemDetails/?modalName=${item.modalName}&orderItemId=${item.orderItemId}&orderId=${item._id}`,
                    {
                        method: "GET",
                        headers:
                        {
