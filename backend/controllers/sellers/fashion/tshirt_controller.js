...
export const AddTShirt = async (req, res) => {

    const { companyName, gender, productType, fit, fabric, pattern, size, sleeve, fabricCare, packOf, neckType, suitableFor } = req.body;
    req.body.comboId = `${companyName}-${gender}-${productType}-${fit}-Fit-${fabric}-${pattern}-${size}-${sleeve}-${packOf}-Tshirt-${neckType}${fabricCare !== '' ? `-${fabricCare}` : ''}${suitableFor !== '' ? `-${suitableFor}` : ''}`.replace(/[.\s()]/g, "-").replace(/--+/g, "-").replace(/---+/g, "-").toLowerCase();


    const data = await Tshirt.create(req.body);

    if (data) {
        return res.status(200).json(
            {
                message: "tshirt is added",
                data: data
            })
    } esle
    {
        return res.status(401).json(
            {
                message: "error in adding tshirt999",
...
export const getAllItemByItemName = async (req, res) => {
    ...
    try {
        ...
        const items = await Tshirt.find({ comboId: req.params.itemName });
...
export const UpdateTShirt = async (req, res) => {
    ...
    const prod = await Tshirt.findById(req.query.prodId);
    ...
    if (prod && prod.seller_ref.equals(req.user.id)) {
        ...
        if (publicIds.length > 0) {
            ...
            const updatedProd = await Tshirt.findByIdAndUpdate(prod._id,
...
export const getAllItemByCompanyName = async (req, res) => {
    ...
    const data = await Tshirt.find({ companyName: req.params.companyName });
    //   console.log(data);
    if (data) {
        return res.status(200).json(
            {
